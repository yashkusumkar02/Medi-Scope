...
class _BrainTumorDetectionPageState extends State<BrainTumorDetectionPage> {
  ...
  Future<List<String>> _loadLabels() async {
    String content = await rootBundle.loadString('assets/labels.txt');
    List<String> labels = content.split('\n');
    return labels.where((label) => label.isNotEmpty).toList();
  }
  Future<void> _pickImageFromGallery() async {
    final img.XFile? pickedFile = await _picker.pickImage(source: img.ImageSource.gallery);
    if (pickedFile != null) {
      setState(() {
  ...
  Future<void> _pickImageFromCamera() async {
    final img.XFile? pickedFile = await _picker.pickImage(source: img.ImageSource.camera);
    if (pickedFile != null) {
      setState(() {
        _image = File(pickedFile.path);
      });
